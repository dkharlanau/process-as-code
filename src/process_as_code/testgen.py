    ...
    for step in data.get("steps", []) or []:
        ...
        if kind not in {"event", "end"}:
            tests.append({
                ...
                "step": sid,
                "type": "functional",
                "scenario": f"Verify '{name}' completes with expected inputs and outputs",
            })
        if kind == "decision":
            for branch in (step.get("branches") or {}).keys():
                tests.append({
                    "id": f"{sid}:branch:{branch}",
                    "step": sid,
                    "type": "branch",
                    "scenario": f"Verify decision '{name}' follows branch '{branch}'",
                })
        for interface in step.get("interfaces", []) or []:
            tests.append({
                "id": f"{sid}:interface:{interface}",
                "step": sid,
                "type": "integration",
                "scenario": f"Verify interface '{interface}' success and failure handling at '{name}'",
            })
        for control in step.get("controls", []) or []:
            tests.append({
                "id": f"{sid}:control:{control}",
                "step": sid,
                "type": "control",
                "scenario": f"Verify control '{control}' is enforced at '{name}'",
            })
    return tests
